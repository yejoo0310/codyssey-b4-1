...
본 프로젝트는 CSS와 JavaScript 코드가 길어지는 문제를 줄이고, 유지보수하기 쉽도록 역할별로 파일을 분리했습니다.
각 파일은 하나의 책임을 중심으로 구성하여, 특정 기능이나 스타일을 수정할 때 관련 파일만 확인할 수 있도록 했습니다.

| 파일 | 역할 |
| --- | --- |
| css/style.css | 역할별로 분리한 CSS 파일들을 불러오는 진입 파일 |
| css/base.css | 전역 기본 스타일 관리 |
| css/layout.css | 공통 레이아웃 스타일 관리 |
| css/components.css | 재사용 가능한 UI 스타일 관리 |
| css/sections.css | 페이지 주요 섹션 스타일 관리 |
| css/responsive.css | 태블릿, 데스크톱 화면에 대응하는 반응형 스타일 관리 |
| js/main.js | 각 기능별 JS 파일을 import하고 초기화 함수를 실행하는 진입 파일 |
| js/dom.js | 여러 JS 파일에서 공통으로 사용하는 DOM 요소를 선택하고 export하는 파일 |
| js/menu.js | 네비게이션 기능 관리 |
| js/theme.js | 다크 모드 전환, localStorage 테마 저장, 시스템 테마 감지 기능 관리 |
| js/animation.js | 스크롤 탑 버튼, 헤더 스크롤 스타일, 부드러운 스크롤, 스크롤 등장 애니메이션, 타이핑 효과 관리 |
| js/contact.js | Contact 폼 유효성 검사와 Formspree 메시지 전송 기능 관리 |
| js/projects.js | GitHub API 호출, 프로젝트 카드 렌더링, 언어별 필터링, 로딩/에러/빈 상태 UI 관리 |
...
### 7. 스크롤 애니메이션
About, Skills, Contact 등 일부 요소에는 `.reveal` 클래스를 적용했습니다.
